...
### 0.1 Data version and time range

The data base of this report is public data accessible on 2026-06-12: official 1.0 Gameplay, official 1.0 Patch Notes, official Help Center Kiosk instructions, official 11.4 Patch Notes, Eternal Return Wiki (Fandom data station) Lumia Island / Research Center / Battle Zone page, DAK.GG character portal, and localized official pictures and character avatar assets. The main text only writes conclusions that can be supported by the data, but there are no links in the main text pile; all sources, evidence boundaries and missing data are concentrated at the end of the text. The unconfirmed coordinates of grass, the precise coordinates of ordinary wild monsters, the radius of footsteps, the size of bunkers and the real death heat map are all written as "needs further verification".

### 0.2 The boundary between image and authenticity

- All images in this report are local paths and do not use remote image links.
- If the official real-life map can be found and localized, priority will be given to using the official real-life map; currently, the real-life maps that can be stably localized and used for regional replacement include the official Mutant Herd map of Beach, Temple, Factory, and Archery Range.
- The remaining areas use the official Kiosk location map, old area cropping map, and conformal map of the accredited research center as real references, and then use image2 for explanatory redrawing: only path arrows, interaction points, risk levels, and visual levels are enhanced. The generated map is not regarded as an official screenshot, and unverified buildings, grass coordinates, footsteps radius, or death heat maps are not added.
- image2 The generated map only enters the text if it meets "structural fidelity": the island outline, the shape of the Research Center red zone, the relative position of the region, the original place name and the Kiosk/teleport point relationship must be consistent with the real reference map. Any pictures that redraw and distort maps, generate typographical place names, or forge new roads or new buildings, even if they are visually more beautiful, will be classified as abandoned maps and will not be used as research evidence. The research center map in this article uses the user-approved fidelity master as the editing target, only allowing image2 to partially redraw the annotation layer without redesigning the map.
- No reliable death heat map, precise coordinates of ordinary wild monsters, grass coordinates, or footsteps material radius were found in the public information; these contents are only explained in the gaps and are not written as facts.
- Only the figure number, pronunciation and data number are retained in the text and illustrations, and external links are not included. For the local files, annotations, supporting opinions and complete sources of each figure, please refer to Sections 10.6 and 11.
...
This report is written based on the actual gameplay of "Eternal Return": it is a PvPvE survival competition/MOBA battle map on Lumia Island. It is not an in-game evacuation map with multiple fixed evacuation points permanently open in the traditional "search, strike and retreat". There are rules related to Escape/Rootkit in the official Game Flow, and this report will discuss them according to the official description; however, the "retreat redemption" commonly used in the text does not generally refer to a fixed evacuation point, but refers to the team's ability to convert the economic and numerical advantages gained into the next rhythm after the battle or target competition through retreat lines, teleport points, kiosks, resurrection parts, Rootkit/Escape judgments, restricted area timers, and next round target positions. All discussions related to Kiosk, Wickeline, Alpha/Omega, mutant beast hordes, restricted zone closure, character routes and regional materials must be supported by official materials, DAK.GG structured route samples, official screenshots or local evidence maps; Battle Zone is only used as 1.0 data to discuss the boundaries of the Wiki historical mechanism, and is no longer used to draw conclusions about the current stable map mechanism; DAK route samples only prove that "the Plan exists on the page and Routes" and does not claim to be an official route, strength ranking, or the single best route.
...
**What it is**: The route card below is not a strength list, nor is it the real-time optimal route. It translates the real experimental body samples used in this report into four more useful questions in level design: what terrain do you like, what terrain are you afraid of, when can you take the initiative to fight, and when should you avoid fighting. The current version of the DAK route sample can already parse the character, weapon, Plan ID, version and region sequence from the page payload; what still needs to be added is the browser screenshot evidence and the redrawing of the old F-04 route diagram according to the real path.
...
## 10. Evidence, graphic lists and data gaps

### 10.0 Delivery Checklist

This table is not to add burden to the main text, but to clearly indicate the degree of completion of the current manuscript: which ones can already be supported by local files and data numbers, and which ones still need to be supplemented with machine diagrams or route screenshots. When continuing to revise, priority will be given to "Partially Completed" and "Uncompleted" items.

| Require | Current status | evidence location | Still need to be strengthened |
|---|---|---|---|
| Three-level argument: what / why / how it affects players | Basically completed | Chapters 1-9; Chapter 5 20 Region by Region Dismantling; Chapter 7 Research Center Special | You can continue to add screenshots of real games in each region to make "how it affects players" more graphic. |
| Based on real information, not fabricated | partially completed | Chapter 0 Data Baseline; Sections 10.1-10.5; Chapter 11 References | The DAK route payload has been able to decode 16 samples; the actual machine station and grass/wall details in the outer ring of the research center still need further verification. |
| Illustrated text and local pictures | Basically completed | Text 53 local images; Section 10.6 Illustration list | Some image2 pictures are still explanatory diagrams and are not equivalent to actual machine evidence; high-definition real machine screenshots should continue to be added in key areas. |
| Real character search road map | partially completed | Section 10.2 has listed 16 DAK payload decoding samples; F-04 is still the old circulation diagram | A screenshot of the DAK page is required, and image2 is redrawn based on the real path to redraw F-04. |
| Hospital/pier/school/forest/research center peripheral blind zone map | partially completed | F-06 Entry Risk Inspection Framework; F-15 Area Remapping; Section 5.21 | The current version of the actual machine diagram is required to mark the walls, grass, door blind spots and ambush windows. |
| Heat map of conflicts in the early, middle and late stages | partially completed | F-05 Mechanism Pressure Diagram; Chapter 6 | Public death/engagement statistics heatmap not found, F-05 cannot be used as a statistics heatmap. |
| Research center special analysis | Basically completed | Chapter 7; F-07 / F-10 / F-15.20; S-06 | There is still a lack of real-life game screenshots of Wickeline's move to the path after Night 4. |
| Character Analysis and Avatars | Basically completed | Chapter 2, Chapter 8, F-08 / F-09 / F-17, S-CHAR | The source of the avatar has been localized; if you want to be more sophisticated, you can continue to add screenshots of the official character page or the source of the current page. |
| Concentrated reference materials at the end of the article | Finish | Chapter 10-11 | The main text should continue to retain only the data number and no external links. |
...
| Haze Assault Rifle / assault rifle | Plan #3107, v11.4.0, path Factory -> Temple, v2 Win 13.30, v2 Like 1717. | Use Factory -> Temple. | 支撑“火力角色从工业成长线转向边角高价值争夺”的解释。 |
...
### 10.5 Data gaps and real machine screenshot collection strategy

**List of pictures to be added**
...
- Screenshot of the route of the current version of DAK.GG Saved Plan: This round has been able to parse 3 routes for each of the 16 sample characters from the DAK SSR payload and `/api/v1/data/areas` dictionary, for a total of 48 Plan IDs, versions, weapons and region sequences; F-04, F-04a, and F-04b have been redrawn according to this set of real network strategy samples. What is still missing is the browser screenshot evidence of the saved plan one by one; until the screenshots are completed, these pictures only prove the existence of structured page samples, but do not prove the route strength ranking or the single optimal route.
...
- "Entrance reading lines" for hospitals, schools, forests, and docks are currently based on map adjacency and Kiosk/target mechanism analysis, and are not equivalent to verified grass points, wall coordinates, or footsteps radius. If you add actual screenshots in the future, you should give priority to replacing the image2 images corresponding to these paragraphs instead of continuing to add abstract images.

**Visual completion status required**

| target icon | Current status | How else to upgrade |
|---|---|---|
...
| Hospital/pier/school/forest/research center peripheral blind zone map | There is an entrance risk inspection framework and regional remapping, but it is not yet evidence of live machine blind spots. | You need actual screenshots or video screenshots of the current version, and mark the wall, grass, door view, and ambush window one by one. |
...
| Research center special structure chart | Already users have approved the conformal master and partial annotation map. | What is most needed is a screenshot of the Wickeline moving outside the path after Night 4 to confirm the boundary performance and team positioning. |
...
| Real machine scene matching map of key locations | Some of the official Mutant Herd scene maps already exist, but key areas are still lacking. | Prioritize the research center, school, hospital, forest, dock and Kiosk resurrection/repair scenes. |
...
| Place | Currently available real images | Proven design points | Still need to add pictures |
|---|---|---|---|
| Hospital / hospital | F-06 入口风险检查图 + F-15.12 地区重绘 | The hospital is a Kiosk/sustainability themed area on the east side. The three-way entrance turns repair and resurrection actions into readable pauses. | Actual screenshots of the three-way entrance to the stream/cemetery/factory, and screenshots of the walls/grass surrounding the Kiosk. |
...
**Real machine screenshot collection and replacement strategy**
...
| S-VID-TODO | Real machine video evidence to be collected | ER Masters / High-level combat video screenshot collection list | To be collected | To be replenished | Video link to be added or local screenshot path | Research center Wickeline battle, school west entrance, hospital three-way entrance, forest wing, dock departure area, Kiosk resurrection, Mutant Herd competition; currently only as a supplementary map collection plan, not as factual evidence |
