...
The data baseline of this report is the public material accessible on 2026-06-12: the official 1.0 Gameplay post, official 1.0 Patch Notes, official Help Center Kiosk article, official 11.4 Patch Notes, Eternal Return Wiki / Fandom pages for Lumia Island, Research Center, and Battle Zone, DAK.GG character route pages, localized official images, and character avatar assets. The main text only makes claims that can be supported by those sources or local files. Source links, evidence boundaries, and non-public data limits are collected at the end so the main reading flow stays clean. Unconfirmed grass coordinates, ordinary monster coordinates, footstep radius, cover dimensions, and real death heatmaps are explicitly excluded from factual conclusions.
